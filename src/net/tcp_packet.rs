...
pub fn create_tcp_packet(buffer: &mut [u8], src_mac: MacAddr, dst_mac: MacAddr, src_ip: Ipv4Addr, dst_ip: Ipv4Addr, src_port: u16, dst_port: u16, flags: u8) -> Result<()> {
    ...
    let mut eth = MutableEthernetPacket::new(&mut buffer[..ETH])
    ...
    let mut ip = MutableIpv4Packet::new(&mut buffer[ETH..ETH + IP])
    ...
    let mut tcp = MutableTcpPacket::new(&mut buffer[ETH + IP..ETH + IP + TCP])
